...
@pytest.fixture
def image_id(lb_url_qa, test_image_path):
    ...
    with open(test_image_path, "rb") as f:
        files = {"file": f}
        response = requests.post(f"http://{lb_url_qa}/image", files=files)
    time.sleep(5)
...
def test_1_get_image_data(lb_url_qa, image_id):
    """ Test for get image data after uploading and recognition. """
    response = requests.get(f"http://{lb_url_qa}/image/{image_id}")
...
def test_2_download_image(lb_url_qa, image_id, image_hash):
    """ Test for downloading image from the app. """
    response = requests.get(f"http://{lb_url_qa}/image/file/{image_id}")
...
def test_3_update_image_data(lb_url_qa, image_id):
    """ Test for updating data for the image. """
    time.sleep(3)
    payload = {"LabelValue": ["test"], "Status": "NEW"}
    response = requests.patch(
        f"http://{lb_url_qa}/image/{image_id}", json=payload
    )

    assert response.status_code == 204


def test_5_delete_image(lb_url_qa, image_id):
    """ Test for deleting image. """
    response = requests.delete(f"http://{lb_url_qa}/image/{image_id}")
    time.sleep(5)
    assert response.status_code == 200
