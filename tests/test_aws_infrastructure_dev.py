...
def test_resources_existence(terraform_data_dev, resource_name, check_function):
    """ Test for ALL resources exists. """
    resource_value = terraform_data_dev[resource_name]
    assert check_function(resource_value),\
        f"Resource {resource_value} does not exist on AWS"


def test_load_balancer_dns(lb_url_dev):
    """ Test for LB url exists. """
    try:
        # check for dns resolving
        socket.gethostbyname(lb_url_dev)
        print(f"Successfully resolved DNS: {lb_url_dev}")

    except socket.gaierror as e:
        assert False, f"DNS resolution failed for {lb_url_dev}: {str(e)}"
